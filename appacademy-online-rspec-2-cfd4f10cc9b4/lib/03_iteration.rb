...
class Array
  def bubble_sort!
    # Without a proc
    for i in (0..self.length-2).reverse_each
      for j in 0..i
        if self[j] > self[j+1]
...
def subwords(word, dictionary)
  dictionary.select {|token| word.include?(token)}
end
...
class Array
  def my_map(&prc)
    result = []
    my_each {|each| result << prc.call(each)}
    result
  end

  def my_select(&prc)
    result = []
    my_each {|each| result << each if prc.call(each)}
    result
  end

  def my_inject(&blk)
    result = self[0]
    self.drop(1).my_each {|each| result = blk.call(result, each)}
    result
  end
end
...
def concatenate(strings)
  strings.inject("") {|concat, el| concat + el}
end
