...
class Array
  def my_uniq
    result = []
    self.each {|num| result << num if !result.include?(num)}
    result
  end
end
...
class Array
  def two_sum
    ...
    self.each_with_index do |num, index|
      theRest = self.drop(index+1)
      # Notice: the second index should be based on the original array
      theRest.each_with_index {|num2, index2| result << [index, index2+index+1] if num + num2 == 0}
    end
    result
  end
end
