...
class Hash
  def difference(other_hash)
    result = {}
    self.each_pair {|key, value| result[key] = value if !other_hash.has_key?(key)}
    other_hash.each_pair {|key, value| result[key] = value if !self.has_key?(key)}
    result
  end
end
...
class Fixnum
  def stringify(base)
    ...
    until num / (base**pow) == 0
      ...
      if digit > 9 #For Hex number letter
        digit = hexLetter[digit-10] #offset calculate
      end
      result.unshift(digit.to_s)
      pow +=1
    end
    result.join('')
  end
